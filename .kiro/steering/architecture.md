...
- Bootstrap flow: `main.js` loads SVGs, creates the root object, starts the Renderer, and calls `initMenu()`. It contains no game logic itself.
- UI wiring: `ui/MenuController.js` (`initMenu()`) binds the menu buttons (New, Save, Load) to their respective actions. "New" delegates to `createGameStage()`, "Save"/"Load" delegate to `dataManager`.
- Game setup: `createGameStage()` in `game/GameStage.js` is the factory that assembles a new game — it creates the GameStage, Hand, Deck, and Panel, wires them together, and registers them on the given parent (typically the root object).
